...
{
    public async Task<Domain.Member.Member?> GetByIdAsync(MemberId id, CancellationToken cancellationToken = default)
        => await context.Members.FirstOrDefaultAsync(m => m.Id == id, cancellationToken);
    ...
    public async Task<bool> IsExistByEmailAsync(MemberId id, string email, CancellationToken cancellationToken)
        => await context.Members.AnyAsync(m => m.Id != id && m.Email.Value == email, cancellationToken);
}